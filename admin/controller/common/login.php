...
class Login extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		// login_captcha extension start
		if($this->config->get('module_login_captcha_status')){
			$this->session->data['captcha'] = substr(sha1(mt_rand()), 17, 6);
			$data['captcha'] = $this->session->data['captcha'];
		}
		// login_captcha extension end
	...
	public function login(): void {
		...
		//  login_captcha extension start
		if($this->config->get('module_login_captcha_status')){
			if (!$json && (empty($post_info['captcha']))) {
				$json['error'] = $this->language->get('error_captcha_missing');
			}

			if (!$json && ($post_info['captcha'] != $this->session->data['captcha'])) {
